...
test("curated journeys use existing internal destinations and distinct real article slugs", () => {
  ...
  for (const [slug, journey] of Object.entries(BLOG_JOURNEYS)) {
    ...
    assert.match(
      journey.href,
      /^\/(?:use-cases|solutions|industries)\/[a-z-]+$|^\/open-source$/,
    );
    assert.ok(
      existsSync(new URL("src/app" + journey.href + "/page.tsx", root)),
      journey.href,
    );
    assert.ok(
      journey.title &&
  ...
  for (const slug of ["not-configured", "toString", "constructor", "__proto__"])
    assert.equal(getBlogJourney(slug), null);
});
...
test("task curation outranks generic category recency without modifying candidates", () => {
  ...
  assert.deepEqual(
    rankRelatedPosts(current, candidates, 3, [
      "specific",
      "specific",
      "missing",
      "current",
    ...
    ["specific", "next-specific", "new-unrelated"],
  );
  assert.deepEqual(
    rankRelatedPosts(current, candidates, 1).map((post) => post.slug),
    ["new-unrelated"],
  );
  assert.equal(
    rankRelatedPosts(current, [...candidates, candidates[0]], 10).length,
    3,
  );
  for (const limit of [0, -1, 1.5, NaN])
    assert.deepEqual(rankRelatedPosts(current, candidates, limit), []);
...
test("related-post integration cannot promote pending, future, or stale-review content", () => {
  const makeArticle = (
    slug,
    { future = false, pending = false, stale = false } = {},
  ) => {
    const data = {
      slug,
      title: slug,
      date: future ? "2099-01-01" : "2026-03-01",
    ...
    if (!pending)
      data.evidenceReview = {
        ...
        contentHash: stale ? "0".repeat(64) : computeContentHash(data, content),
      };
  ...
  const files = {
    ...
    "stale.md": makeArticle("best-ai-voice-agent-platforms-2026", {
      stale: true,
    }),
  ...
  assert.deepEqual(
    blog.getRelatedPosts("vapi-alternatives", 3).map((post) => post.slug),
    ["eligible-fallback"],
  );
});
...
test("priority article edits keep exact reviews and contextual workflow/pricing links", () => {
  for (const [slug, journey] of Object.entries(BLOG_JOURNEYS)) {
    ...
    assert.ok(
      isValidEvidenceReview(post.evidenceReview, post.contentHash),
      slug,
    );
    assert.ok(
      post.content.includes("](" + journey.href + ")"),
      slug + " workflow link",
    );
    assert.ok(post.content.includes("](/pricing)"), slug + " pricing link");
    assert.ok(
      post.content.includes("](/company/contact)"),
      slug + " enquiry link",
    );
    const substantivePropertyUpdate =
      slug === "ai-voice-agents-property-management" ||
      slug.startsWith("property-management-") ||
      slug === "after-hours-leasing-call-handling";
    if (substantivePropertyUpdate)
      assert.equal(
      ...
      assert.equal(
        post.updatedAt,
        "2026-09-06",
        "link-only edits do not manufacture freshness",
      );
  }
});
...
test("reviewed article SSR has a task-specific journey, resources, contextual recommendations and one H1", async () => {
  ...
  const post = articles.find((candidate) => candidate.slug === slug);
  const related = BLOG_JOURNEYS[slug].relatedSlugs.map((relatedSlug) =>
    articles.find((candidate) => candidate.slug === relatedSlug),
  );
  const makePage = (indexable) =>
    loadTs("src/app/blog/[slug]/page.tsx", {
      "@/lib/blog": {
        getPostBySlug: () => post,
        getRelatedPosts: () => related,
        isIndexablePost: () => indexable,
        getPostModifiedDate: () => post.updatedAt,
      },
    });
  const html = renderToStaticMarkup(
    await makePage(true).default({ params: Promise.resolve({ slug }) }),
  );
  assert.equal((html.match(/<h1/g) ?? []).length, 1);
  assert.match(html, /aria-labelledby="article-next-step"/);
  assert.match(
    html,
    /href="\/use-cases\/appointment-scheduling" data-analytics-location="article_journey"/,
  );
  assert.match(
    html,
    /href="\/company\/contact" data-analytics-location="article_journey"/,
  );
  assert.match(html, /href="\/resources"/);
  assert.match(html, /BlogPosting/);
  assert.doesNotMatch(html, /Editorial content under evidence review/);
  const pending = renderToStaticMarkup(
    await makePage(false).default({ params: Promise.resolve({ slug }) }),
  );
