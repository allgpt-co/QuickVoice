...
**Automation-first rule:** If Codex CAN do it via CLI/API, Codex MUST do it. Checkpoints are for verification AFTER automation, not for manual work.

### Task Sizing

Each task should take Codex **15-60 minutes** to execute.
...
**The test:** Could a different Codex instance execute this task without asking clarifying questions? If not, add specificity.
...
Record in `user_setup` frontmatter. Only include what Codex literally cannot do (account creation, secret retrieval, dashboard config).
...
Only include what Codex literally cannot do (account creation, secret retrieval, dashboard config).
